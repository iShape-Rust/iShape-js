...
const operationButtons = Array.from(document.querySelectorAll(".curve-motion [data-operation]"));
...
bindRangeOutput(sizeInput, requireElement("curve-motion-size-output", HTMLOutputElement), (value) => `${value}%`);
bindRangeOutput(speedInput, requireElement("curve-motion-speed-output", HTMLOutputElement), (value) => `${(value / 100).toFixed(1)}×`);
bindRangeOutput(toleranceInput, requireElement("curve-motion-tolerance-output", HTMLOutputElement), (value) => (value / 1000).toFixed(3));
let operationName = "Intersect";
...
function createScene() {
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        subject.push(createBlob(centerX + orbitX, centerY + orbitY, baseRadius, baseRadius * (0.84 + 0.05 * Math.sin(seed)), segmentCount, lobes, phase * 1.7 + seed, 0.11));
        clip.push(createBlob(centerX - orbitX, centerY - orbitY, baseRadius * 0.92, baseRadius * (0.88 + 0.04 * Math.cos(seed)), segmentCount, lobes + 1, -phase * 1.45 + seed * 0.73, 0.095));
    }
    return { subject, clip };
}
function createBlob(centerX, centerY, radiusX, radiusY, segmentCount, lobes, radialPhase, ripple) {
...
function blobSample(centerX, centerY, radiusX, radiusY, lobes, radialPhase, ripple, angle) {
...
function addCubicContour(path, contour) {
...
function updateStats(subject, clip, result, resultData) {
...
function countCurvedSegments(shapes) {
    let count = 0;
...
function recordSolveSample(value) {
...
//# sourceMappingURL=motion_lab.js.map