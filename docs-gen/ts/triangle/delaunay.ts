...
(() => {
    ...
    function findPoint(x: number, y: number): Point | null {
    ...
    function drawCircle(context: CanvasRenderingContext2D): void {
        ...
        const x =
            (magnitudeA * (by - cy) + magnitudeB * (cy - ay) + magnitudeC * (ay - by)) /
            denominator;
        const y =
            (magnitudeA * (cx - bx) + magnitudeB * (ax - cx) + magnitudeC * (bx - ax)) /
            denominator;
