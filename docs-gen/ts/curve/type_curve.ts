...
type DragState =
    | {kind: "handle"; object: EditorObject; handle: PointHandle}
    | {kind: "object"; object: EditorObject; previous: Point};
...
let glyphs: EditorObject[] = [];
let selectedObjectId: string | null = null;
let operationName: OperationName = "Union";
let fillRuleName: FillRuleName = "EvenOdd";
let renderedFontSize = DEFAULT_FONT_SIZE;
let dragState: DragState | null = null;
let hoverHandle: PointHandle | null = null;
...
function commandsToContours(commands: PathCommand[]): PathContour[] {
    const contours: PathContour[] = [];
    let contour: PathContour | null = null;
...
function buildGeometry(objects: readonly EditorObject[]): CurveGeometry {
...
function countSegments(objects: readonly EditorObject[]): number {
    return objects.reduce((total, object) => total + object.contours.reduce(
        (contourTotal, contour) => contourTotal + contour.segments.length,
        0,
    ), 0);
}

function addBuilderSegment(builder: CurveBuilder, object: EditorObject, segment: PathSegment): void {
    if (segment.type === "line") {
        ...
        builder.bezierCurveTo(
            ...toWorld(object, segment.ctrl0),
            ...toWorld(object, segment.ctrl1),
            ...toWorld(object, segment.to),
        );
    }
}
...
function objectToPath(object: EditorObject): Path2D {
...
function addPathSegment(path: Path2D, object: EditorObject, segment: PathSegment): void {
    if (segment.type === "line") {
...
function drawControlLines(object: EditorObject): void {
    ctx.beginPath();
    object.contours.forEach((contour) => {
...
function controlLine(object: EditorObject, from: Point, to: Point): void {
    ctx.moveTo(...toWorld(object, from));
    ctx.lineTo(...toWorld(object, to));
}

function drawHandles(object: EditorObject): void {
...
function objectHandles(object: EditorObject): PointHandle[] {
...
function findHandle(object: EditorObject, worldPoint: Point): PointHandle | null {
    const handles = objectHandles(object);
    const localPoint: Point = [worldPoint[0] - object.position[0], worldPoint[1] - object.position[1]];
...
function selectObject(object: EditorObject): void {
...
function toWorld(object: EditorObject, point: Point): Point {
